...
class SearchResultsController extends Controller
{
    ...
    public function showAjax(Request $request)
    {
        ...
        if($cat!=null && $cat!=-1){
            $searchResults =  Item::where("category_id",$cat);
            if($q!=null)
                $searchResults =  $searchResults->whereRaw('item.search @@ plainto_tsquery(?)', array(strtolower($q)))->whereRaw($priceQuerryString)//->where("price", '>', $min_price)->where("price", '<', $max_price)
                ->orderByRaw('ts_rank(item.search, plainto_tsquery(?)) DESC', array(strtolower($q)));
            $searchResults = $searchResults->where("is_archived",false)->get();
        }
        else
            $searchResults =  Item::whereRaw('item.search @@ plainto_tsquery(?)', array(strtolower($q)))
            ->orderByRaw('ts_rank(item.search, plainto_tsquery(?)) DESC', array(strtolower($q)))->where("is_archived",false)->whereRaw($priceQuerryString)//->where("price", '>', $min_price)->where("price", '<', $max_price)
            ->get();
